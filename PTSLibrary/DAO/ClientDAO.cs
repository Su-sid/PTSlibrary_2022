...
namespace PTSLibrary.DAO
{
    class ClientDAO : SuperDAO
    {
        public TeamLeader Authenticate(string username, string password)
        {
            ...
            sql = String.Format("SELECT DISTINCT Person.Name, UserId, TeamId FROM Person INNERJOIN Team ON(Team.TeamLeaderId = Person.UserId) WHERE Username = '{0}' AND Password = '{1}'", username, password);
           
            cn = new SqlConnection(Properties.Resources.ConnectionString);
